...
def prepare_canvas(resized_img, target_size, target_w, target_h):
    ...
    # FIX 6: Use black (0.0) instead of gray (0.3) for the canvas background.
    # A 0.3 fill gives BCE(0.3, 0.3) ≈ 0.62 per padding pixel — a constant loss
    # floor that drowns out the reconstruction signal from the plate itself.
    # Black (0.0) contributes near-zero BCE loss when the model correctly outputs 0,
    # so gradient energy is concentrated on the actual plate region.
    ...
    # FIX 6: Removed the contrast stretch  (canvas − mean) × 1.15 + mean.
    # That operation pushed tails outside [0,1] before clipping, turning smooth
    # grayscale gradients into near-binary values and shifting the pixel
    # distribution away from what the model expects at inference time.
